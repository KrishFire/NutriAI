...
if (fs.existsSync(entitlementsPath)) {
  ...
  if (entitlements.includes('com.apple.developer.speechrecognition')) {
    ...
    console.log('❌ Speech recognition entitlement NOT found in entitlements file');
  }
} else {
...
if (fs.existsSync(infoPlistPath)) {
  ...
  if (infoPlist.includes('NSSpeechRecognitionUsageDescription')) {
    ...
    console.log('❌ Speech recognition usage description NOT found in Info.plist');
  }
} else {
...
console.log('2. Apple\'s App ID needs to be updated to include Speech Recognition capability');
console.log('3. This happens automatically during EAS build, but may take 1-2 build attempts');
