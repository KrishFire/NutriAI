...
function checkEnvironment() {
  ...
  if (!hasTestEnv) {
    ...
    log(
      '   MSW integration tests will run, but real user experience tests may fail',
      'yellow'
    );
    log(
      '   Copy .env.test.example to .env.test and configure for full testing',
      'yellow'
    );
  } else {
  ...
  const requiredDeps = ['@testing-library/react-native', 'jest', 'msw'];
  ...
  if (missingDeps.length > 0) {
    ...
    log(
      '   Run: npm install --save-dev @testing-library/react-native msw',
      'yellow'
    );
    process.exit(1);
  } else {
...
function runTests(testType, hasRealEnv) {
  ...
  if (testType === 'real' && !hasRealEnv) {
    log(
      '⚠️  Skipping real user experience tests - no .env.test configuration',
      'yellow'
    );
    return true;
  }
...
function generateReport(results) {
  ...
  if (passed === total) {
    log(
      '\n🎉 All tests passed! Your app should work correctly for users.',
      'green'
    );
  } else {
    log(
