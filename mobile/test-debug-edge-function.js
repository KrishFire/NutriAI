...
const supabaseUrl = process.env.SUPABASE_URL || 'https://cdqtuxepvomeyfkvfrnj.supabase.co';
const supabaseAnonKey = process.env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNkcXR1eGVwdm9tZXlma3ZmcjNqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTk2MDg5OTMsImV4cCI6MjAzNTE4NDk5M30.F-i4a_Wh1P2YajCfEw-xf0YrZBnXy6MHPF0_AuC7aHg';
...
async function testDebugFunction() {
  ...
  try {
    const response = await fetch(`${supabaseUrl}/functions/v1/food-search-debug`, {
      method: 'POST',
      headers: {
  ...
  try {
    ...
    try {
      ...
      console.log(`Original function response preview: ${responseText.substring(0, 300)}...\n`);
