...
describe('Food Search Edge Function Forensic Analysis', () => {
  ...
  describe('Step 3: Payload and Input Validation Tests', () => {
    ...
    test('should test with complete payload', async () => {
      ...
      try {
        ...
        const responseText = await response.text();
        console.log(
          `📊 Complete payload response body:`,
          responseText.substring(0, 500)
        );
    ...
    test('should test edge function environment validation', async () => {
      ...
      try {
        ...
        if (responseText.includes('USDA API key')) {
          console.log(
            '🎯 FOUND THE ISSUE: USDA API key is missing or not configured!'
          );
          console.log(
            '🔧 This is likely the root cause of the "non-2xx status code" error'
          );
        }
      } catch (error) {
  ...
  describe('Step 4: Direct Edge Function URL Tests', () => {
    test('should test direct curl-equivalent request', async () => {
      ...
      try {
        ...
        console.log(
          `📊 Direct request response headers:`,
          Object.fromEntries(response.headers.entries())
        );

        const responseText = await response.text();
        console.log(
          `📊 Direct request response body:`,
        ...
        if (response.status >= 400) {
          ...
          console.log(
            '🔍 This is the "non-2xx status code" error that the client is seeing!'
          );
          ...
          try {
            ...
            if (
              ...
              console.log(
                '🚨 ROOT CAUSE IDENTIFIED: USDA API key is not configured in edge function environment!'
              );
            }
          } catch {
  ...
  describe('Step 5: Root Cause Analysis Summary', () => {
    test('should summarize findings and provide fix recommendations', () => {
      ...
      console.log(
        `   • Authentication token: ${authToken ? 'Available' : 'Not available'}`
      );

      console.log('\n🎯 MOST LIKELY ROOT CAUSE:');
      console.log(
        '   The edge function is failing because the USDA_API_KEY environment variable'
      );
      console.log(
        '   is not configured in the Supabase edge function environment.'
      );

      console.log('\n🔧 REQUIRED FIXES:');
      console.log('   1. Configure USDA_API_KEY in Supabase project secrets');
      console.log(
        '   2. Redeploy the edge function to pick up the new environment variable'
      );
      console.log('   3. Test the function again with a valid API key');

      console.log('\n📝 EVIDENCE:');
      console.log(
        '   • The edge function code checks for USDA_API_KEY and returns 500 if missing'
      );
      console.log(
        '   • The error "non-2xx status code" suggests a 500-level server error'
      );
      console.log(
        '   • No recent logs indicate the function is not being called or is crashing silently'
      );
...
export async function debugEdgeFunctionDirectly(
  authToken: string,
  query: string = 'apple'
) {
