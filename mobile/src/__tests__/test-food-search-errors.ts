...
async function testRateLimit() {
  ...
  results.forEach((result, index) => {
    if (result.status === 'fulfilled') {
      ...
      if (!error) {
        successCount++;
      } else if (error.message?.includes('rate limit') || error.details?.stage === 'rate-limiting') {
        rateLimitCount++;
        if (rateLimitCount === 1) {
  ...
  console.log(`Results: ${successCount} success, ${rateLimitCount} rate limited, ${otherErrorCount} other errors`);
}
