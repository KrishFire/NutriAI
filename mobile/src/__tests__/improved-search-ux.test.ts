...
describe('Improved Search UX', () => {
  ...
  describe('Progressive Disclosure', () => {
    ...
    it('should prioritize common foods over branded products', async () => {
      ...
      const brandedFoods = result.groups.find(
        g => g.title === 'Branded Products'
      );
      ...
      const commonScores = commonFoods!.items.map(
        item => item.relevanceScore || 0
      );
      const brandedScores = brandedFoods!.items.map(
        item => item.relevanceScore || 0
      );

      expect(Math.min(...commonScores)).toBeGreaterThan(
        Math.max(...brandedScores)
      );
    });
    ...
    it('should separate cooking ingredients into their own category', async () => {
      ...
      const ingredients = result.groups.find(
        g => g.title === 'Cooking Ingredients'
      );
  ...
  describe('UX Improvements Validation', () => {
    it('should solve the chicken search problem', async () => {
      ...
      const initialDisplayCount = result.groups.reduce(
      ...
      const ingredientsGroup = result.groups.find(
        g => g.title === 'Cooking Ingredients'
      );
