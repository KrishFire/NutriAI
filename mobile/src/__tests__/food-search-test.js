...
class FoodSearchTester {
  ...
  async testDirectEdgeFunctionCall() {
    ...
    try {
      ...
      this.log(
        `Raw response - Error: ${error ? JSON.stringify(error) : 'null'}`
      );
      this.log(
        `Raw response - Data: ${data ? JSON.stringify(data, null, 2) : 'null'}`
      );
    ...
    try {
      ...
      if (!error && data) {
        ...
        if (data.foods && data.foods.length > 0) {
          ...
          validationResults.foodHasCalories =
            typeof firstFood.calories === 'number';
          validationResults.foodHasProtein =
            typeof firstFood.protein === 'number';
  ...
  generateFinalReport() {
    ...
    const searchTests = this.testResults.filter(
      r =>
        r.testName === 'Multiple Query Test' ||
        r.testName === 'Direct Edge Function Call'
    );
    const searchSuccessRate =
      (searchTests.filter(r => r.success).length / searchTests.length) * 100;
    this.log(
      `FOOD SEARCH FUNCTIONALITY: ${searchSuccessRate.toFixed(1)}% success rate`
    );

    // Response validation
    const structureTests = this.testResults.filter(
      r => r.testName === 'Response Structure Validation'
    );
    const structureSuccessRate =
      (structureTests.filter(r => r.success).length / structureTests.length) *
      100;
    this.log(
      `RESPONSE STRUCTURE VALIDATION: ${structureSuccessRate.toFixed(1)}% success rate`
    );
    ...
    if (failedTests === 0) {
      ...
      this.log(
        '✅ Edge Function is responding correctly with proper data structure'
      );
    } else if (searchSuccessRate >= 80) {
      this.log(
        '⚠️  MOSTLY FUNCTIONAL - Core functionality works with some edge cases'
      );
      this.log('✅ Food search Edge Function is operational');
    } else {
      this.log(
        '❌ SIGNIFICANT ISSUES DETECTED - Food search needs investigation'
      );
      this.log(
        '❌ Edge Function may have authentication or structural problems'
      );
    }
