...
console.log(`${colors.cyan}=== Edge Function Error Transformation Verification ===${colors.reset}\n`);
...
async function runTests() {
  console.log(`${colors.blue}Testing USDA API directly to understand error responses...${colors.reset}\n`);
  ...
  for (const scenario of testScenarios) {
    ...
    const statusColor = result.status === scenario.expectedUsdaStatus ? colors.green : colors.red;
    console.log(`${statusColor}Status: ${result.status} ${result.statusMessage}${colors.reset}`);
    console.log(`Response: ${result.data}\n`);
  }

  console.log(`${colors.magenta}Key Finding:${colors.reset}`);
  console.log('When USDA API returns 403/401, the Edge Function code at line 496 throws:');
  console.log(`${colors.red}throw new Error(\`USDA API error \${response.status}: \${errorText}\`);${colors.reset}`);
  ...
  console.log('  constructor(public status: number, public statusText: string, message: string) {');
  ...
  console.log('throw new USDAAPIError(response.status, response.statusText, errorText);');
  console.log('');
  console.log('// Lines 727-737 become:');
  console.log('} catch (error) {');
  console.log('  log("usda-api-error", { ');
  console.log('    error: error.message,');
  console.log('    status: error instanceof USDAAPIError ? error.status : undefined');
  console.log('  });');
  console.log('  ');
  ...
  console.log('          // Include hint for developers (not shown to end users)');
  console.log('          hint: "Check USDA_API_KEY configuration"');
  console.log('        }),');
  console.log('        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }');
  console.log('      );');
  console.log('    }');
  console.log('    ');
  ...
  console.log('          error: "Food database rate limit exceeded. Please try again later.",');
  console.log('          requestId');
  console.log('        }),');
  console.log('        { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } }');
  console.log('      );');
  console.log('    }');
  console.log('  }');
  console.log('  ');
  ...
  console.log('      error: "Failed to search food database. Please try again.",');
  console.log('      requestId');
  console.log('    }),');
  console.log('    { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } }');
  console.log('  );');
  console.log('}');
  console.log('```');
}
