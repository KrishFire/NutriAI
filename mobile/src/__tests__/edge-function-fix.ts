...
async function handleUSDAError(error: unknown, requestId: string) {
      ...
      case 403:
        ...
        return new Response(
          JSON.stringify({
            ...
            error: 'Food database service configuration error. Please contact support.',
            requestId,
            // Debug info for developers (could be omitted in production)
            debug: {
              hint: 'Check USDA_API_KEY environment variable',
              originalError: error.status === 403 ? 'Invalid or missing API key' : 'Unauthorized'
            }
          }),
          { 
      ...
      case 429:
        ...
        return new Response(
          JSON.stringify({
            ...
            error: 'Food database rate limit exceeded. Please try again in a few moments.',
            requestId,
      ...
      case 404:
        ...
        return new Response(
          JSON.stringify({
            ...
            error: 'Food database service endpoint error. Please contact support.',
            requestId
          }),
          { 
      ...
      case 503:
        ...
        return new Response(
          JSON.stringify({
            ...
            error: 'Food database service is temporarily unavailable. Please try again later.',
            requestId
          }),
          { 
  ...
  return new Response(
    JSON.stringify({
      ...
      error: 'Unable to connect to food database. Please check your connection and try again.',
      requestId
    }),
    { 
      status: 502, // Bad gateway
