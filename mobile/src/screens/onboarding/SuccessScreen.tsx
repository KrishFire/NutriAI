...
const MacroProgressBar = ({ label, value, maxValue, color, percentage }: { 
  label: string; 
  value: number; 
  maxValue: number; 
  color: string;
...
export function SuccessScreen({ onContinue }: SuccessScreenProps) {
  ...
  const totalMacroCalories = (macros.carbs * 4) + (macros.protein * 4) + (macros.fat * 9);
  const carbPercentage = Math.round((macros.carbs * 4 / totalMacroCalories) * 100);
  const proteinPercentage = Math.round((macros.protein * 4 / totalMacroCalories) * 100);
  const fatPercentage = Math.round((macros.fat * 9 / totalMacroCalories) * 100);
  
  // Calculate milestone dynamically based on user goal
  const currentWeight = parseFloat(userData.weight?.value || userData.currentWeight?.value || '150');
            ...
            <View className="flex-1 justify-center">
              ...
              >
                ...
                <View className="flex-row justify-between items-start mb-6">
                  <View>
                    <Text className="text-xs text-gray-600 dark:text-gray-400">Daily Calories</Text>
                    <Text className="text-2xl font-bold text-gray-900 dark:text-white">{dailyCalories}</Text>
                  </View>
              ...
              >
                <Text className="text-sm text-gray-900">
                  ...
                  {milestoneWeight} lbs in {milestoneWeeks} week{milestoneWeeks > 1 ? 's' : ''}
                </Text>
              </MotiView>
            </View>
